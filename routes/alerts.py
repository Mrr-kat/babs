...
def get_alertas():
    ...
    # 3. Prediction-based alerts (will run out < 7 days)
    productos = conn.execute("SELECT * FROM productos WHERE activo=1 AND stock_actual > 0").fetchall()
    for p in productos:
        ventas = conn.execute("""
            SELECT COALESCE(SUM(dv.cantidad),0) as qty
            FROM detalle_ventas dv JOIN ventas v ON dv.venta_id=v.id
            WHERE dv.producto_id=? AND v.fecha >= date('now','-14 days')
        """, (p['id'],)).fetchone()['qty']
    ...
    # 4. High margin products with low stock (opportunity)
    oportunidades = conn.execute("""
        SELECT p.*, c.nombre as categoria,
               ROUND((p.precio_venta - p.precio_compra)*100.0/NULLIF(p.precio_venta,0),1) as margen
        FROM productos p LEFT JOIN categorias c ON p.categoria_id=c.id
